...
def mask_account_card(account_card: str) -> str:
    ...
    if "Счет" in account_card:
        account_number = parts[-1] # используем последний элемент из списка parts
        return f"Счет **{account_number[-4:]}" # выводим маскированный номер счёта
    else:
